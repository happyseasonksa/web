          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary" style="text-align: right;">
                <div class="card-header">
                    <h3 class="card-title" style="float: right">{{__('Add New Category')}}</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form method="POST" action="{{route('admin.category.store')}}" role="form" id="add-category" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        <div class="form-group">
                            <label for="name">{{__('Name')}}</label>
                            <input type="text" name="name" class="form-control @error('name'){{'is-invalid'}}@enderror" value="{{old('name')}}" id="name" placeholder="{{__('Enter Name')}}">
                            @error('name')
    ...
    $(document).ready(function () {
      ...
      $('#add-category').validate({
        ...
        messages: {
            ...
            icon: {
                required: "Please select a driving licence image",
